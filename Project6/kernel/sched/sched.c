...
const ptr_t s_pid0_stack = INIT_KERNEL_STACK_S + 2 * PAGE_SIZE;
pcb_t s_pid0_pcb = {
    .pid = 0,
...
pid_t do_exec(char *name, int argc, char *argv[]){
    ...
    pcb[i].pgdir = pcb[i].pgdir_ori;
    clear_pgdir(pcb[i].pgdir);

    share_pgtable(pcb[i].pgdir, pa2kva(PGDIR_PA));

    uint64_t entry_point = load_task_img(name, pcb[i].pgdir);
    if(entry_point == 0) return 0;
    else{
        pcb[i].kernel_sp   = ROUND(pcb[i].kernel_sp  , PAGE_SIZE);
        pcb[i].user_sp     = ROUND(pcb[i].user_sp    , PAGE_SIZE);
        pcb[i].user_sp_kva = ROUND(pcb[i].user_sp_kva, PAGE_SIZE);
        map_page_helper(pcb[i].user_sp-PAGE_SIZE, kva2pa(pcb[i].user_sp_kva-PAGE_SIZE), pcb[i].pgdir);
        pcb[i].pid         = i + 1;
        pcb[i].status      = TASK_READY;
        pcb[i].cursor_x    = 0;
        pcb[i].cursor_y    = 0;
        pcb[i].cpu_mask    = current_running[cpu_id]->cpu_mask;
        pcb[i].user_sp_kva-= sizeof(char*)*argc;
        pcb[i].user_sp    -= sizeof(char*)*argc;
        argv_ptr           = (char**)pcb[i].user_sp;
        argv_ptr_kva       = (char**)pcb[i].user_sp_kva;
        ...
        for(int j = argc -1; j >= 0; j--){
            ...
            pcb[i].user_sp_kva -= len;
            pcb[i].user_sp     -= len;
            argv_ptr_kva[j]     = (char *)pcb[i].user_sp;
            strcpy((char *)pcb[i].user_sp_kva , argv[j]);
        }
        pcb[i].user_sp = (reg_t)ROUNDDOWN(pcb[i].user_sp, 128);
        init_pcb_stack(pcb[i].kernel_sp, pcb[i].user_sp, entry_point, &pcb[i], argc, argv_ptr);
        Queue_push(&pcb[i].list, &ready_queue);
    }
    return pcb[i].pid;
}
...
void do_exit(void){
    uint64_t cpu_id = get_current_cpu_id();
    // printk("PID %d EXITED unexpectedly!\n", current_running[cpu_id]->pid);
...
int do_waitpid(pid_t pid){
    ...
    // 2. 循环等待，直到子进程状态变为 TASK_EXITED
    // 只要子进程还在跑（RUNNING, READY, BLOCKED），父进程就必须阻塞
    while (child->status != TASK_EXITED) {
        // 设置当前进程为阻塞状态
        current_running[cpu_id]->status = TASK_BLOCKED;

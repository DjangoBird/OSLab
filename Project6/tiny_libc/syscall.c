...
void sys_write_ch(char ch){
    invoke_syscall(SYSCALL_WRITECH, (long)ch, IGNORE, IGNORE, IGNORE, IGNORE);
}
...
void sys_condition_destroy(int cond_idx)
{
    ...
    invoke_syscall(SYSCALL_BARR_DESTROY, cond_idx, IGNORE, IGNORE, IGNORE, IGNORE);
}
...
int sys_taskset(int mode, int mask, void *arg)
{
    return invoke_syscall(SYSCALL_TASKSET, (long)mode, (long)mask, (int)(long)arg, IGNORE,IGNORE);
}

pid_t  sys_thread_create(uint64_t entry_point, uint64_t arg)
{
    /* TODO: [p3-task1] call invoke_syscall to implement sys_exec */
    return invoke_syscall(SYSCALL_THREAD_CREATE, (uintptr_t)entry_point, (uintptr_t)arg, IGNORE, IGNORE, IGNORE);
}
...
{
    return invoke_syscall(SYSCALL_PIPE_OPEN, (uintptr_t)name, IGNORE, IGNORE, IGNORE,IGNORE);
}

long sys_pipe_give_pages(int pipe_idx, void *src, size_t length)
{
    return invoke_syscall(SYSCALL_PIPE_GIVE, pipe_idx, (uintptr_t)src, length, IGNORE, IGNORE);
}

long sys_pipe_take_pages(int pipe_idx, void *dst, size_t length)
{
    return invoke_syscall(SYSCALL_PIPE_TAKE, pipe_idx, (uintptr_t)dst, length, IGNORE, IGNORE);
}
...
int sys_mkfs(int force_flag)
{
    ...
    return invoke_syscall(SYSCALL_FS_MKFS, (long)force_flag, IGNORE, IGNORE, IGNORE, IGNORE);  // sys_mkfs succeeds
}
...
int sys_net_recv_stream(void *buffer, int *nbytes, int length)
{
    return invoke_syscall(SYSCALL_NET_RECV_STREAM, (long)buffer, (long)nbytes, (long)length, IGNORE, IGNORE);
}
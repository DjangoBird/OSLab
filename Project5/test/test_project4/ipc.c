...
static void fill_payload1(char *buf, size_t len) {
  for (size_t i = 0; i < len; ++i) {
    buf[i] = (char)(i & 0x00);
  }
}
...
static int mailbox_sender(void) {
	...
	int sent = 0;
	char string = '1';
	for (int i = 0; i < MSG_BYTES / 4096; i++) {
		//string = buf[i * 4096];
		sent += sys_mbox_send(mq, &buf[i * 4096], 4096);
	}
...
static int mailbox_receiver(void) {
	...
	for (int i = 0; i < MSG_BYTES / 4096; i++) {
		//buf[i * 4096] = '1';
		received += sys_mbox_recv(mq, &buf[i * 4096], 4096);
	}
	...
	for (size_t i = 1; i < MSG_BYTES; i+=4096) {

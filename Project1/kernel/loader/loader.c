...
uint64_t load_task_img(char *taskname)
{
    ...
    for(i=0;i<TASK_MAXNUM;i++){
        if(strcmp(tasks[i].task_name, taskname)==0){
            ...
            bios_sd_read(entry_addr, tasks[i].block_nums, start_sector);//扇区数自适应
            return entry_addr + (tasks[i].start_addr - start_sector * 512);//返回程序的存储位置
            //由于entry_addr是按TASK_SIZE对齐的，而实际程序可能不是（有非零偏移）
        }
    }
    //没找到

...
int main(void)
{
    ...
    while (1)
    {
        ...
        sys_set_sche_workload(CHECK_POINT - 0);
	...
	printf("%c", '|');
        ...
        for (int i = (60 - LENGTH) * CYCLE_PER_MOVE; i < 60 * CYCLE_PER_MOVE; i++)
        {
            ...
            if(i % CYCLE_PER_MOVE == 0)
            {
                ...
                int current_position = LENGTH - remain_length;
                int workload;
                if(current_position < CHECK_POINT){
                    workload = CHECK_POINT - current_position;
                }else{
                    workload = (LENGTH - current_position);
                }
                sys_set_sche_workload(workload);
            }
        }
        // sys_yield();
        sys_move_cursor(0, j);
